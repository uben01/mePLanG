...
public class check_RoutineCallParameters_NonTypesystemRule extends AbstractNonTypesystemRule_Runtime implements NonTypesystemRule_Runtime {
  ...
  public void applyRule(final SNode call, final TypeCheckingContext typeCheckingContext, IsApplicableStatus status) {
    if (!(Objects.equals(ListSequence.fromList(SLinkOperations.getChildren(call, LINKS.parameters$nM0V)).count(), ListSequence.fromList(SLinkOperations.getChildren(SLinkOperations.getTarget(call, LINKS.routine$RDrw), LINKS.inputs$jb5C)).count()))) {
    ...
    for (int i = 0; i < ListSequence.fromList(SLinkOperations.getChildren(call, LINKS.parameters$nM0V)).count(); i++) {
      if (!(Objects.equals(TypecheckingFacade.getFromContext().getTypeOf(ListSequence.fromList(SLinkOperations.getChildren(call, LINKS.parameters$nM0V)).getElement(i)) + "", SLinkOperations.getTarget(ListSequence.fromList(SLinkOperations.getChildren(SLinkOperations.getTarget(call, LINKS.routine$RDrw), LINKS.inputs$jb5C)).getElement(i), LINKS.type$pLrO) + ""))) {
        final MessageTarget errorTarget = new NodeMessageTarget();
        IErrorReporter _reporter_2309309498 = typeCheckingContext.reportTypeError(ListSequence.fromList(SLinkOperations.getChildren(call, LINKS.parameters$nM0V)).getElement(i), "A paraméter típusa nem megfelelő", "r:cf47ddf0-7871-49fa-b23f-152a01cabfaa(mePLanG.typesystem)", "4847857255823971288", null, errorTarget);
      }
    }
  }
  public SAbstractConcept getApplicableConcept() {
    return CONCEPTS.RoutineCallExpression$8D;
  }
